...
class TaskResourceResolver(ABC):
    ...
    async def download_files(self, task: Task, task_io_manager: TaskIOManager):
        ...
        for name, url in files.items():
            ...
            name, obj = await self.decode_from_bytes(name, byte_array)
            task.files[name] = obj
...
class TaskExectionInfo(object):
    ...
    def __init__(self, type_name: str,
        ...
        if not issubclass(self.resource_resolver, TaskResourceResolver):
            raise TypeError(f"Task resource resolver must be a subclass of TaskResourceResolver instead of {self.resource_resolver}")
